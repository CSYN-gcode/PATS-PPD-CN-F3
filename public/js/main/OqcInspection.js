...
function GetOqcInspectionById(getPo,
	...
	$.ajax({
        ...
        success: function(response){
            ...
            $('#txtOqcInspectionMaterialName').val(getMaterialCode)
            
            if(getProductionRuncardData[0].drawing_no != ''){
                $('#txtBDrawingNo').val(getProductionRuncardData[0].drawing_no)
                $('#txtBDrawingRevision').val(getProductionRuncardData[0].drawing_rev)
                $('#btnOqcInspectionViewBDrawings').prop('disabled', false)
            }else{
                $('#btnOqcInspectionViewBDrawings').prop('disabled', true)
            }
            ...
            if(getOqcInspectionData.length > 0){
                ...
                let percentage = (getProductionRuncardData[0].ship_output-countDefects)/getProductionRuncardData[0].ship_output*100
