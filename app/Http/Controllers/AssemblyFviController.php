...
class AssemblyFviController extends Controller
{
    public function view_visual_inspection(Request $request){
        ...
        ->addColumn('ttlLotQty', function($fvi_data){
            ...
            ->join('production_runcards', 'assembly_fvis_runcards.prod_runcard_id', '=', 'production_runcards.id')
            ->join('production_runcard_stations', 'assembly_fvis_runcards.prod_runcard_station_id', '=', 'production_runcard_stations.id')
            ->select(
                DB::raw('SUM(production_runcard_stations.output_quantity) as ttl_output'),
                DB::raw('SUM(production_runcard_stations.ng_quantity) as ttl_ng'),
            )
    ...
    public function view_fvi_runcards(Request $request){
        ...
        ->select(
            ...
            'production_runcards.id AS fk_runcard_id',
            'production_runcards.production_lot AS fk_runcard_runcard_no',
            'production_runcard_stations.input_quantity AS fk_runcard_station_input_quantity',
            'production_runcard_stations.output_quantity AS fk_runcard_station_output_quantity',
            'production_runcard_stations.ng_quantity AS fk_runcard_station_ng_quantity',
        )
        ...
        ->addColumn('mods', function($fvi_runcards){
            ...
            ->table('production_runcard_station_mods')
            ->join('defects_infos', 'production_runcard_station_mods.id', '=', 'defects_infos.id')
            ->select(
                'production_runcard_station_mods.*',
                'defects_infos.defects AS fk_defect_name'
            )
            ->where('production_runcard_station_mods.prod_runcards_id', $fvi_runcards->prod_runcard_id)
            ->where('production_runcard_station_mods.prod_runcards_id', $fvi_runcards->prod_runcard_station_id)
            ->whereNull('production_runcard_station_mods.deleted_at')
            ->get();
    ...
    public function get_runcard_details(Request $request){
        ...
        ->join('production_runcard_stations', 'production_runcards.id', '=', 'production_runcard_stations.prod_runcards_id')
        ->join('stations', 'production_runcard_stations.station', '=', 'stations.id')
        ->select(
            'production_runcards.*',
            'production_runcard_stations.id AS fk_station_id',
            'production_runcard_stations.station AS fk_station_station',
            'production_runcard_stations.input_quantity AS fk_station_input_quantity',
            'production_runcard_stations.output_quantity AS fk_station_output_quantity',
            'stations.station_name AS fk_station_name'
        )
        ->where('production_runcards.production_lot', $request->production_lot)
        ...
        ->where('production_runcard_station_mods.prod_runcards_id', $runcard_details->id)
        ->where('production_runcard_station_mods.prod_runcards_id', $runcard_details->fk_station_id)
        ->whereNull('production_runcard_station_mods.deleted_at')
        ->get();
